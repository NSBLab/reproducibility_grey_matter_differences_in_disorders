% function binarize_tmap_brainsmash_null_func(iNull, iCOMBAT, smoothKernel)
...
for iDiag = 1:nDiag
    ...
    sigFwemapSurrs_P_HC = zeros(sum( mask>0,'all'),1);
        % read the tmap
        binarymap1 = niftiread(['/projects/kg98/trangc/VBM/data/derivatives/s' ,char(num2str(smoothKernel)), 'COMBAT/' ,char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0001_binary.nii']);
        fwemap1 = niftiread(['/projects/kg98/trangc/VBM/data/derivatives/s' ,char(num2str(smoothKernel)), 'COMBAT/' ,char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0001_binary_fwe.nii']);
        binarymap2 = niftiread(['/projects/kg98/trangc/VBM/data/derivatives/s' ,char(num2str(smoothKernel)), 'COMBAT/' ,char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0002_binary.nii']);
        fwemap2 = niftiread(['/projects/kg98/trangc/VBM/data/derivatives/s' ,char(num2str(smoothKernel)), 'COMBAT/' ,char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0002_binary_fwe.nii']);
        ...
        save([address,'/',char(diagnosisString),'/',char(siteString(iSite)),'/binary_surrogate_maps.mat'], 'mapAllNull', 'sigmapSurrs_HC_P', 'sigmapSurrs_P_HC', 'sigFwemapSurrs_HC_P', 'sigFwemapSurrs_P_HC')

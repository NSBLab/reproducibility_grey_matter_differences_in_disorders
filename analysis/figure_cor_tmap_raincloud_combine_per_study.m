...
s_study = load(['output/corr_per_study_tmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'.mat'], ...
    'cor1', 'cor2','corThres1','corThres2','repThres1','repThres2', ...
    "siteList",'siteThresList','t1All','t2All','thresmap1','thresmap2');
...
iData = plotorder(iDiag); % the order in the data
  ...
  corToPlot{5,iDiag} = s_all.repThres1{iData}(ids);
   ...
   nStudyToPlot = length(s_study.cor1{iData});
    corDiagcombinePerStudy = [];
    corHC_PcombinePerStudy = [];
    repHC_PcombinePerStudy = [];

      for iStudy = 1:nStudyToPlot
        ids = find(tril(ones(size(s_study.cor1{iData}{iStudy})),-1));
    
    corDiagcombinePerStudy = [corDiagcombinePerStudy; s_study.cor1{iData}{iStudy}(ids)];
     corHC_PcombinePerStudy = [corHC_PcombinePerStudy; s_study.corThres1{iData}{iStudy}(ids)];
     repHC_PcombinePerStudy = [repHC_PcombinePerStudy; s_study.repThres1{iData}{iStudy}(ids)];
      end
...
%%
savefig(fig,['output/figure_corr_tmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_per_study.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', ['output/figure_corr_tmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_per_study.jpg'])

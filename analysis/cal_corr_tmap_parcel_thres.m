function [cor1, cor2,rowst1_without_zeros, rowst2_without_zeros, siteList] = cal_corr_tmap_parcel_thres(address, metadata, diagnosisString, mask)
...
for iSite = 1:nSite
    t1 = niftiread(['/projects/kg98/trangc/VBM/data/', address, char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0001_binary.nii']);
    t2 = niftiread(['/projects/kg98/trangc/VBM/data/', address, char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0002_binary.nii']);
...
siteList = siteString(rowst1_without_zeros);
end

...
[cor1{iSite}, cor2{iSite}, rowst1_without_zeros{iSite}, rowst2_without_zeros{iSite}] = cal_corr_tmap_thres(address, metadata, diagnosisString(iSite));

end

save('output/dice.mat', 'cor1', 'cor2')

% if size(cor1,1)>1
%     ids=find(triu(ones(size(cor1)),1));
%     [fi xi]=ksdensity(cor1(ids),'function','pdf','npoints',200);
% 
...
%     hm = heatmap(siteString(rowst1_without_zeros), siteString(rowst1_without_zeros), cor1,'Interpreter','none','ColorLimits',[0 0.1], 'CellLabelColor', 'None');
%     hm.Colormap = parula;
...
%         'FontName',font_name,'FontSize',font_size, 'horizontalalignment', 'left');
%     hm = heatmap(siteString(rowst1_without_zeros), siteString(rowst1_without_zeros), cor1,'Interpreter','none','ColorLimits',[0 0.5], 'CellLabelColor', 'None');
%     % title(['Correlation between t-maps HC > P in ', diagnosisString(iSite)]);
%     hm.Colormap = parula;
%     % title(['Correlation between t-maps HC > P in ', diagnosisString(iSite)]);
% 

...
for iPara = 1:nPara
    for iDiag = 1:nDiag
        for iParaVar = 1:nParaVar
            ...
            nullMedian = median(corToPlot{2+(iParaVar-1)*2, iDiag});
            % Calculate percentile (i.e., proportion of null values less than or equal to empirical median)
            p_value(iPara,iParaVar,iDiag) = mean(abs(empiricalMedian - nullMedian  ) <= abs(corToPlot{2+(iParaVar-1)*2, iDiag}-nullMedian));
...
for iRow = 1:numRow
    ...
    idx = reshape(bsxfun(@plus, (iRow-1)*2, [1 2 9 17])',1,[])  % generate index pairs
    ...
    for iPara = 1:nPara-1
        for iDiag = 1:nDiag
            if p_value(iPara,iRow,iDiag) <= thres
                a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.022+(iPara-1)*0.065+(iDiag-1)*0.167), ax2.Position(2)+ax2.Position(4)*0.85, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
    ...
    for iDiag = 1:nDiag
        if p_value(iPara,iRow,iDiag) <= thres
            a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.022+0.1+(iDiag-1)*0.167), ax2.Position(2)+ax2.Position(4)*0.85, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
                'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'left','Interpreter','none');
        end
    end

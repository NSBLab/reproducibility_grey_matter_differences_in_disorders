function [cor1, cor2,rowst1_without_zeros, rowst2_without_zeros] = cal_corr_tmap_null_thres(address, metadata, diagnosisString, mask,iNull)
...
for iSite = 1:nSite
    t1 = niftiread([address,num2str(iNull),'/', char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0001_binary.nii']);
    t2 = niftiread([address,num2str(iNull),'/', char(diagnosisString),'/',char(siteString(iSite)),'/spmT_0002_binary.nii']);

...
medFile = readtable(['/projects/kg98/trangc/VBM/data/', study, '/medlist01.txt']);
hisFile = readtable(['/projects/kg98/trangc/VBM/data/', study, '/psychistory01.txt']);
medName = readtable(['/projects/kg98/trangc/VBM/data/medication_name_for_code.csv']);
% medFile.src_subject_id = medFile.src_subject_id(:,2:end);
imageFile.src_subject_id = cellstr((imageTable.src_subject_id));
imageFile.image_description = cellstr(imageTable.image_description);
...
siteFile.src_subject_id = cellstr((siteTable.src_subject_id));
siteFile.phenotype_description = cellstr(siteTable.phenotype_description);

% image filename
pat1 = "submission"+wildcardPattern+".zip";
...
imageFile.useID = ismember(imageFile.src_subject_id,useID)& ismember(imageFile.image_description,{'MRI','mprage_5e_32ch','mprage_5e_32ch_RMS'});
...
useFolderName = cellfun(@(x) [char(x(2:end))],matchID, 'UniformOutput', false);
...
metadata.site = cellstr(repmat('39',size(indexHighQRinUseFolder)));
...
metadata.antipsychotic = if_med(matchIDHighQR, medFile, medName, 2, 10);  %maxNoMed=18
 metadata.moodstabiliser = if_med(matchIDHighQR, medFile, medName, 3, 10);
metadata.antidepression = if_med(matchIDHighQR, medFile, medName, 4, 10);
metadata.antianxiety = if_med(matchIDHighQR, medFile, medName, 5, 10);
...
metaTable = cell2table([metadata.subj_id(metadata.con), metadata.dataset(metadata.con), ...
    ...
    cellstr(num2str(metadata.antianxiety(metadata.con))), metadata.treatment(metadata.con), cellstr(num2str(metadata.ageOnset)),...
    cellstr(num2str(metadata.illnessDuration))], "VariableNames",["subj_id", "dataset", "site", "diagnosis",...
    "age", "sex", "site_string", "sex_string",  "diagnosis_string", "EN"  ,"antipsychotic", "moodstabiliser", "antidepression", "antianxiety","treatment","ageOnset","illnessDuration"]);
...
imageTable(ismember(cellstr(imageTable.src_subject_id),{['M',testsub1.subj_id{1}(5:end)],['M',testsub2.subj_id{1}(5:end)]}),:)
siteFile.phenotype_description(ismember(cellstr(siteFile.src_subject_id),{['M',testsub1.subj_id{1}(5:end)],['M',testsub2.subj_id{1}(5:end)]}))
siteFile.src_subject_id(ismember(cellstr(siteFile.src_subject_id),{['M',testsub1.subj_id{1}(5:end)],['M',testsub2.subj_id{1}(5:end)]}))

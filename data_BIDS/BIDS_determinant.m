...
diagList = {'HC','BD','SCA','SCZ','MDD'}
nDiag = length(diagList);
diagIndex = {[17,18,19,21,22,25,34,35,36],[2:16],[28:30],[31:33,37],[22,23]};
...
for iSub = 1:length(useFolder)
    ...
    for iFile = 1:length(filePerSub)
        if ~ismember(datePerSub(iFile),uniqueDate)
            ...
            mkdir(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolder(iSub)),'/anat' ]);
            copyfile(['/scratch/kg98/Data_Trang/MDD_BP_SCZ_determinant/image03/',char(filePerSub(iFile))],['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolder(iSub)),'/anat/sub-',char(useFolder(iSub)),'_T1w.nii.gz'  ])
            gunzip(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolder(iSub)),'/anat/sub-',char(useFolder(iSub)),'_T1w.nii.gz'  ]);
            delete(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolder(iSub)),'/anat/sub-',char(useFolder(iSub)),'_T1w.nii.gz'  ]);

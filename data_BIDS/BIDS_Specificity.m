...
imageFile.src_subject_id = cellstr(arrayfun(@(x) num2str(sprintf('%02d',x)), imageTable.src_subject_id(2:end,:), 'UniformOutput', false));
...
for iSub = 1:length(useFolder)
    ...
    for iFile = 1:length(filePerSub)
        if ~ismember(datePerSub(iFile),uniqueDate)
            ...
            copyfile(['/scratch/kg98/Data_Trang/BD_Specificity_Hippocampal_Subregion/image03/',char(filePerSub(iFile))],['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'.nii.gz'  ])
       gunzip(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'.nii.gz'  ]);
      movefile(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'.nii'  ],...
          ['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'_T1w.nii'  ]);
      delete(['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'.nii.gz'  ]);

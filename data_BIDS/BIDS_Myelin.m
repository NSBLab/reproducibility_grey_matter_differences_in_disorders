...
allItems = dir(['/scratch/kg98/Data_Trang/MDD_myelin/',char(useFolder(iSub))]);
...
allFolders = allFolders(~ismember({allFolders.name}, {'.', '..'}));  % Exclude '.' and '..'
    ...
    for iFile = 1:height(allFolders)
            ...
            copyfile(['/scratch/kg98/Data_Trang/MDD_myelin/',char(useFolder(iSub)),'/',allFolders(iFile).name,'/anat/',char(useFolder(iSub)),'_',allFolders(iFile).name,'_T1w.nii.gz'],...
           ...
           copyfile(['/scratch/kg98/Data_Trang/MDD_myelin/',char(useFolder(iSub)),'/',allFolders(iFile).name,'/anat/',char(useFolder(iSub)),'_',allFolders(iFile).name,'_T1w.json'],...
                ['/projects/kg98/trangc/VBM/data/', study,'/',char(useFolder(iSub)),'/ses-',char(num2str(iFile)),'/anat/',char(useFolder(iSub)),'_','ses-',char(num2str(iFile)),'_T1w.json'  ]);

...
useFolderName = cellfun(@(x) [char(x(2:end))],useFolder, 'UniformOutput', false);
...
for iSub = 1:length(useFolder)
    ...
    for iFile = 1:length(filePerSub)
        if ~ismember(datePerSub(iFile),uniqueDate)
            ...
            copyfile(['/scratch/kg98/Data_Trang/BP_MultidimCog/image03/',char(filePerSub(iFile))],['/projects/kg98/trangc/VBM/data/', study,'/sub-',char(useFolderName(iSub)),'/anat/sub-',char(useFolderName(iSub)),'.zip'  ])

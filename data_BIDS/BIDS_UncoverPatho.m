...
imageFile.src_subject_id = cellstr(cellfun(@(x) x, imageTable.src_subject_id(2:end,:), 'UniformOutput', false));
...
siteFile.src_subject_id = cellstr(cellfun(@(x) x, siteTable.src_subject_id, 'UniformOutput', false));
siteFile.phenotype = cellstr(siteTable.phenotype);

% STEP 3: put the corect image filename according to info file
pat1 = "Users"+wildcardPattern + (".tar.gz");
...
% STEP 4: pause to choose the phenotype from diag and rerun
diagIndex = {[2,3],[1],[],[4],[],[]};
...
% STEP 5: choose the file description as in info file

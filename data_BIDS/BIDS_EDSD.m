...
imageTable = readtable(['/projects/kg98/trangc/VBM/data/', study, '/ng2_EDSD-fulldataset.csv']);
...
for iSub = 1:length(useFolder)
   ...
   if exist(['/scratch/kg98/Data_Trang/EDSD/',char(useFolder(iSub)),'_ses-T00_acq-3D_T1w.nii.gz'])
       useID(iSub) = 1;
        %     useFolder(iSub)
        %     mkdir(['/projects/kg98/trangc/VBM/data/', study,'/',char(useFolder(iSub)),'/ses-1/anat' ]);
        %     copyfile(['/scratch/kg98/Data_Trang/EDSD/',char(useFolder(iSub)),'_ses-T00_acq-3D_T1w.nii.gz'],...

...
if strcmp(study,'ABIDEII')
medFile = readtable(['/projects/kg98/trangc/VBM/data/', study, '/dem/ABIDEII-BNI_1.csv']);
end
...
metadata.site = cellstr(num2str(round(subject.site(iUseFile(indexHighQRinUseFolder)))));
...
if strcmp(study, 'ABIDEI') 
metadata.site = cellstr(num2str(Lb+31));
else
    metadata.site = cellstr(num2str(Lb+32));
end
...
if strcmp(study,'ABIDEII')
%% meditcation
[La indexIDinMed] = ismember(cellstr(extract(metadata.subj_id(metadata.con),digitsPattern)), cellstr(num2str(medFile.SUB_ID)));
positiveIn = find(indexIDinMed>0);
metadata.treatment = cellstr(num2str(medFile.CURRENT_MED_STATUS(indexIDinMed(positiveIn)) == 1));
...
metaTable = cell2table([metadata.subj_id(metadata.con), metadata.dataset(metadata.con), ...
    ...
    metadata.diagnosis_string(metadata.con), metadata.CAT, metadata.treatment], "VariableNames",["subj_id", "dataset", "site", "diagnosis",...

...
datasets = ["ABIDEI","ABIDEII","ASRB","FEMS","KANMDD","MITASD","OCDPG","RUSMDD","SPAINOCD","WASHASD","YMDD"]
%datasets = ["KANMDD","MITASD","OCDPG","RUSMDD","SPAINOCD","WASHASD","YMDD"]
%datasets = ["RUSMDD","SPAINOCD","WASHASD","YMDD"];
...
for d=1:length(datasets)
    ...
    % Directories
    inDir = '/scratch/kg98/Ashlea/projects/Trang/';
    subDir = ['/scratch/kg98/Ashlea/parcellation_v2/subjlists/'];
    outDir = [inDir, 'derivatives/COMBAT/' ];

    % Load in metadata
    metadataFilename = [subDir, dataset, '_dems.csv'];
    ...
    for i=1:size(metadata,1)
        ...
        subNifti = ['/scratch/kg98/Ashlea/parcellation_v2/datadir/',dataset,'/',subj_id,'/anat/mri/mwp1',subj_id, '_T1w.nii'];
        subReport = ['/scratch/kg98/Ashlea/parcellation_v2/datadir/',dataset,'/',subj_id,'/anat/report/cat_',subj_id, '_T1w.xml'];
    ...
    subReport_cell = subReport_cell'
     ...
     % Smooth nifti images
    if smoothData == 1 
        smooth_job(subNifti_cell)
    end 

    % Calculate TIV
    if calculateTIV == 1 
        tiv_filename = [outDir, dataset, '_TIV_combat.txt']
    ...
    for i=1:size(metadata,1)
        subj_id = metadata.subj_id{i}
        subNiftiSmooth = ['/scratch/kg98/Ashlea/parcellation_v2/datadir/',dataset,'/',subj_id,'/anat/mri/s8mwp1',subj_id, '_T1w_combat.nii'];
    ...
    for s = 1:numSite
        ...
        for i=1:numPatGroups % For each unique patient group
            ...
            factorial_design_ttest_job(newSubFolder, hcCell, patCell, age, sex, tiv)

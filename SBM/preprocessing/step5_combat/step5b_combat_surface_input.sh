#!/bin/bash
# Dispatcher: reads combat groups from config, submits step5b_sub_combat_surface_input.sh per group

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SUB_SCRIPT="${SCRIPT_DIR}/step5b_sub_combat_surface_input.sh"
...
for GROUP in $COMBAT_GROUPS; do
    echo "=== Submitting step5b for group: $GROUP ==="
    export CONFIG_FILE GROUP HPC_ENABLED

    if [[ "$HPC_ENABLED" == "1" ]]; then
        sbatch --job-name="step5b_${GROUP}" "$SUB_SCRIPT"
    else
        bash "$SUB_SCRIPT"
    fi
done

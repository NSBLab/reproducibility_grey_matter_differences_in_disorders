#!/bin/bash
# Dispatcher: reads combat groups from config, submits step5d_sub_combat_surface_output.sh per group

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SUB_SCRIPT="${SCRIPT_DIR}/step5d_sub_combat_surface_output.sh"
...
for GROUP in $COMBAT_GROUPS; do
    echo "=== Submitting step5d for group: $GROUP ==="
    export CONFIG_FILE GROUP HPC_ENABLED

    if [[ "$HPC_ENABLED" == "1" ]]; then
        sbatch --job-name="step5d_${GROUP}" "$SUB_SCRIPT"
    else
        bash "$SUB_SCRIPT"
    fi
done

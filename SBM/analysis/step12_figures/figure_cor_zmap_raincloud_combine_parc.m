...
for iDiag = 1:nDiag
  iData = plotorder(iDiag); % the order in the data
    ...
    corToPlot{2,iDiag} = corzmapSurrsDKAll{iData};

    ids{iDiag}=find(tril(ones(size(corDiagSF100{iData})),-1));
    corToPlot{3,iDiag} = corDiagSF100{iData}(ids{iDiag});
    corToPlot{4,iDiag} = corzmapSurrsSF100All{iData};

    ids{iDiag}=find(tril(ones(size(corDiagSF500{iData})),-1));
    corToPlot{5,iDiag} = corDiagSF500{iData}(ids{iDiag});
    corToPlot{6,iDiag} = corzmapSurrsSF500All{iData};

    ids{iDiag}=find(tril(ones(size(corDiagSF1000{iData})),-1));
    corToPlot{7,iDiag} = corDiagSF1000{iData}(ids{iDiag});
    corToPlot{8,iDiag} = corzmapSurrsSF1000All{iData};

    ids{iDiag}=find(tril(ones(size(corSigDK{iData})),-1));
    corToPlot{9,iDiag} = corSigDK{iData}(ids{iDiag});
    corToPlot{10,iDiag} = corsigmapSurrsDKAll{iData};

    ids{iDiag}=find(tril(ones(size(corSigSF100{iData})),-1));
    corToPlot{11,iDiag} = corSigSF100{iData}(ids{iDiag});
    corToPlot{12,iDiag} = corsigmapSurrsSF100All{iData};

    ids{iDiag}=find(tril(ones(size(corSigSF500{iData})),-1));
    corToPlot{13,iDiag} = corSigSF500{iData}(ids{iDiag});
    corToPlot{14,iDiag} = corsigmapSurrsSF500All{iData};

    ids{iDiag}=find(tril(ones(size(corSigSF1000{iData})),-1));
    corToPlot{15,iDiag} = corSigSF1000{iData}(ids{iDiag});
    corToPlot{16,iDiag} = corsigmapSurrsSF1000All{iData};

    ids{iDiag}=find(tril(ones(size(repSigDK{iData})),-1));
    corToPlot{17,iDiag} = repSigDK{iData}(ids{iDiag});
    corToPlot{18,iDiag} = repsigmapSurrsDKAll{iData};

    ids{iDiag}=find(tril(ones(size(repSigSF100{iData})),-1));
    corToPlot{19,iDiag} = repSigSF100{iData}(ids{iDiag});
    corToPlot{20,iDiag} = repsigmapSurrsSF100All{iData};

    ids{iDiag}=find(tril(ones(size(repSigSF500{iData})),-1));
    corToPlot{21,iDiag} = repSigSF500{iData}(ids{iDiag});
    corToPlot{22,iDiag} = repsigmapSurrsSF500All{iData};

    ids{iDiag}=find(tril(ones(size(repSigSF1000{iData})),-1));
    corToPlot{23,iDiag} = repSigSF1000{iData}(ids{iDiag});
    corToPlot{24,iDiag} = repsigmapSurrsSF1000All{iData};
...
for iRow = 1:numRow
    ...
    if iRow == 1
        ...
        annotation('line', [x(2) x(2)], [y+0.01 y], 'Color', 'k', 'LineWidth', 1);
        a17 = annotation(fig, 'textbox', [ax3.Position(1)-0.01, ax3.Position(2)+0.02, ax3.Position(3)*2, ax3.Position(4)], 'string', 'SF500', 'edgecolor', 'none', ...
            'FontName',font_name,'FontSize',fontsize_legend,  'horizontalalignment', 'left','Interpreter','none');
        ...
        ax3= annotation('line', [x(1) x(2)], [y+0.01 y+0.01], 'Color', 'k', 'LineWidth', 1);
        annotation('line', [x(2) x(2)], [y+0.01 y], 'Color', 'k', 'LineWidth', 1);
        a17 = annotation(fig, 'textbox', [ax3.Position(1)-0.01, ax3.Position(2)+0.02, ax3.Position(3)*2, ax3.Position(4)], 'string', 'SF1000', 'edgecolor', 'none', ...
            'FontName',font_name,'FontSize',fontsize_legend,  'horizontalalignment', 'left','Interpreter','none');

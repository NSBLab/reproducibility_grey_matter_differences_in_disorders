function figure_cor_zmap_raincloud_combine_lh_rh(config)
...
for iPara = 1:nPara
    for iDiag = 1:nDiag
        if p_value(iPara,iDiag) <= thres
            a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.045+(iPara-1)*2*0.031+(iDiag-1)*0.166), ax2.Position(2)+ax2.Position(4)*0.65, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
...
a17 = annotation(fig, 'textbox', [ax3.Position(1), ax3.Position(2), ax3.Position(3), ax3.Position(4)], 'string', paralist{1}, 'edgecolor', 'none', ...
...
y = ax2.Position(2)+ax2.Position(4)*0.88;        % normalized vertical location

% Draw three lines to make the bracket
annotation('line', [x(1) x(1)], [y y+0.03], 'Color', 'k', 'LineWidth', 1);
ax4 = annotation('line', [x(1) x(2)], [y+0.03 y+0.03], 'Color', 'k', 'LineWidth', 1);
annotation('line', [x(2) x(2)], [y+0.03 y], 'Color', 'k', 'LineWidth', 1);
a17 = annotation(fig, 'textbox', [ax4.Position(1), ax4.Position(2)+0.07, ax4.Position(3), ax4.Position(4)], 'string', paralist{2}, 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'center');

%%
savefig(fig,fullfile(output_dir, ['figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_lh_rh.fig']));
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', fullfile(output_dir, ['figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'lh_rh.jpg']))
end

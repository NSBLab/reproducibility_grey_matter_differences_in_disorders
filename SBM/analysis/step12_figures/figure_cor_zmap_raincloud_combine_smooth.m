function figure_cor_zmap_raincloud_combine_smooth(config)
...
for iDiag = 1:nDiag
    if p_value(iPara,iDiag) <= thres
        a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.02+(iPara-1)*0.047+(iDiag-1)*0.155), ax2.Position(2)+ax2.Position(4)*0.6, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
...
varCor =  cellfun(@(x) var(x), corToPlot);
stdCor =  cellfun(@(x) std(x), corToPlot);
...
y = ax2.Position(2)+ax2.Position(4)*0.93;        % normalized vertical location

% Draw three lines to make the bracket
annotation('line', [x(1) x(1)], [y y+0.03], 'Color', 'k', 'LineWidth', 1);
ax4 = annotation('line', [x(1) x(2)], [y+0.03 y+0.03], 'Color', 'k', 'LineWidth', 1);
annotation('line', [x(2) x(2)], [y+0.03 y], 'Color', 'k', 'LineWidth', 1);
a17 = annotation(fig, 'textbox', [ax4.Position(1), ax4.Position(2)+0.07, 0.1, ax4.Position(4)], 'string', [char(num2str(paralist(3))),' mm'], 'edgecolor', 'none', ...
            'FontName',font_name,'FontSize',fontsize_legend,  'horizontalalignment', 'left');

%%
savefig(fig,fullfile(output_dir, ['figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_smooth_combine.fig']));
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', fullfile(output_dir, ['figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_smooth_combine.jpg']))
end

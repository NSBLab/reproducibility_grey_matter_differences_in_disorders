...
load(['output/corr_zmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_',hemi,'_all.mat'], 'map', ...
    'corSigFdrHC_P','corSigFdrP_HC','repSigFdrHC_P','repSigFdrP_HC','siteList')
...
switch vari
    case 'corHC_P'
        corSigFdr = corSigFdrHC_P;
        corsigFdrmapSurrsP_HCVerAll = corsigFdrmapSurrsHC_PVerAll;
        map.sigFdrmap = map.sigFdrmapHC_P;
    case 'corP_HC'
        corSigFdr = corSigFdrP_HC;
        corsigFdrmapSurrsP_HCVerAll = corsigFdrmapSurrsHC_PVerAll;
        map.sigFdrmap = map.sigFdrmapP_HC;
    case 'repHC_P'
        corSigFdr = repSigFdrHC_P;
        corsigFdrmapSurrsP_HCVerAll = repsigFdrmapSurrsHC_PVerAll;
        map.sigFdrmap = map.sigFdrmapHC_P;
    case 'repP_HC'
        corSigFdr = repSigFdrP_HC;
...
for iDiag = 1:nDiag
    ...
    if maxCor ~= 0
        [rowMax colMax] = find(round(maxCor,9)==round(corSigFdr{iDiag},9));
        ...
        for iSite = 1:length(colLeft)
            ...
            patch(ax3, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', map.sigFdrmap(iMap,:)', ...
...
if markColorbar == 0
    ...
    if (sum(corToPlot{1,iDiag}==0) == 1 & minCor == 0) | (minCor ~= 1 & minCor ~= 0)
        [rowMin colMin] = find(round(minCor,9)==round(corSigFdr{iDiag},9));
        ...
        for iSite = 1:length(colRight)
            ...
            patch(ax5, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', map.sigFdrmap(iMap,:)', ...
                'EdgeColor', 'none', 'FaceColor', 'interp');

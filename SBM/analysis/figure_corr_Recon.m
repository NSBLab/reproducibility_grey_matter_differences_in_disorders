function figure_corr_Recon(map, nEig)
...
ids=find(triu(ones(size(corDiagRecon{plotOrder(iDiag)})),1));
[fi xi]=ksdensity(corDiagRecon{plotOrder(iDiag)}(ids),'function','pdf','npoints',200);
...
hm = imagesc(1:length(siteString), 1:length(siteString), triu(corDiagRecon{plotOrder(iDiag)},1));
clim([-0.2 1]);
...
a1 = annotation(fig, 'textbox', [0, 0.98, 0.1, 0.01], 'string', num2str(nEig), 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size, 'horizontalalignment', 'left');
end
...
%%
% savefig(fig,['figure_corr_recon.fig']);
% set(fig, 'PaperPositionMode', 'auto')
% print(fig, '-djpeg', '-r600', 'figure_corr_recon.jpg')
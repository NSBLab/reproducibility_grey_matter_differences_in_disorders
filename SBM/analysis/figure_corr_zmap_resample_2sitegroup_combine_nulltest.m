...
diag = 2;
hemis = 'lh';
dataDir = fullfile('/scratch','kg98','trangc','VBM','data', 'derivatives', 'freesurfer',['s',num2str(smoothKernel),'noCOMBAT'],['diag',num2str(diag)],hemis, 'resample_2sitegroup');
...
for iFolder = 1:height(subdivideList)
    %
    divideMat = load(fullfile(dataDir,subdivideList(iFolder).name,'corr_surface_nulltest.mat'));
...
%     axis image;
    ...
    for iSampleSize=1:length(sampleSizeList)
        ...
        matFile = load(fullfile(contrastDir,'corr_surface_nulltest.mat'));

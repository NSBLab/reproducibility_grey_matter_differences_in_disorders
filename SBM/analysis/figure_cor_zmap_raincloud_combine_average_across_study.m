...
s_study = load(['output/corr_average_across_study_zmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_',hemi,'_all.mat'], 'map','corDiag','corSigHC_P', 'repSigHC_P' )
...
for iDiag = 1:nDiag
    ...
    corToPlot{5,iDiag} = s_all.repSigHC_P{iData}(ids);
     ...
     corToPlot{2,iDiag} = s_study.corDiag(iData,:);
    corToPlot{4,iDiag} = s_study.corSigHC_P(iData,:);
    corToPlot{6,iDiag} = s_study.repSigHC_P(iData,:);
...
maxCor = cellfun(@(x) max(x), corToPlot);
  ...
  % Convert data coordinates to normalized figure units
    ...
    y = ax2.Position(2)+ax2.Position(4)*0.4;        % normalized vertical location
...
%%
savefig(fig,['output/figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_averagre_across_study.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', ['output/figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_average_across_study.jpg'])

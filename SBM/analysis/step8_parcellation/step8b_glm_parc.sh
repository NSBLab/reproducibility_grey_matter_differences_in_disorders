#!/bin/bash
# Step8b: GLM on parcellated SBM maps (MATLAB).
...
matlab -nodisplay -r "addpath('$SCRIPT_DIR'); step8b_glm_parc('$CONFIG_FILE', '$hemi'); quit;"
if [ $? -ne 0 ]; then
    echo "Error: step8b_glm_parc failed"
    exit 1
fi
echo "=== STEP8B DONE ==="

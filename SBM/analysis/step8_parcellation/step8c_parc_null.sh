#!/bin/bash
# Step8c: parcellate SBM eigentrapping null maps (MATLAB).
...
matlab -nodisplay -r "addpath('$SCRIPT_DIR'); step8c_parc_null('$CONFIG_FILE', '$hemi'); quit;"
if [ $? -ne 0 ]; then
    echo "Error: step8c_parc_null failed"
    exit 1
fi
echo "=== STEP8C DONE ==="

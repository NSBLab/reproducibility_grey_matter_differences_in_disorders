function step10b_corr_zmap_var_sex(config, hemi, iter, type)
% STEP10B: Sex counts and sex ratio between sites vs z-map correlation.
% Usage: step10b_corr_zmap_var_sex(config [, hemi, iter, type])
% Prereq: step9a. Writes: confound_sex.mat under results/SBM/analysis/output/.
...
load(corrMat, 'map', 'corDiag', 'siteList');

% --- Mantel test per diagnosis ---
varTable = cell(nDiag, 1);
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, maleRatio{iDiag}, femaleRatio{iDiag}, subjectRatio{iDiag}, maleFemaleRatio{iDiag}, nSite{iDiag}] = ...
        cor_var_sex(metadata, diagnosisString(iDiag), corDiag{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_sex.mat'), 'varTable', 'maleRatio', 'femaleRatio', 'subjectRatio', 'maleFemaleRatio', 'nSite');
end

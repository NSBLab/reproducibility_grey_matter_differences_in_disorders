function step10g_corr_zmap_var_illness_duration(config, hemi, iter, type)
% STEP10G: Illness duration mean/variance between sites vs z-map correlation.
% Usage: step10g_corr_zmap_var_illness_duration(config [, hemi, iter, type])
% Prereq: step9a. Writes: confound_illnessDuration.mat under results/SBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, meanIllness{iDiag}, varIllness{iDiag}, nSite{iDiag}] = ...
        cor_var_illness(metadata, diagnosisString(iDiag), corDiag{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_illnessDuration.mat'), 'varTable', 'meanIllness', 'varIllness', 'nSite');
end

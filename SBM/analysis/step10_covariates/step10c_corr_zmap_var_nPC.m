function step10c_corr_zmap_var_nPC(config, hemi, iter, type)
% STEP10C: Patient/HC/sample counts between sites vs z-map correlation.
% Usage: step10c_corr_zmap_var_nPC(config [, hemi, iter, type])
% Prereq: step9a. Writes: confound_nPC.mat under results/SBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, patientRatio{iDiag}, controlRatio{iDiag}, subjectRatio{iDiag}, patientControlRatio{iDiag}, nSite{iDiag}] = ...
        cor_var_nPC(metadata, diagnosisString(iDiag), corDiag{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_nPC.mat'), 'varTable', 'patientRatio', 'controlRatio', 'subjectRatio', 'patientControlRatio', 'nSite');
end

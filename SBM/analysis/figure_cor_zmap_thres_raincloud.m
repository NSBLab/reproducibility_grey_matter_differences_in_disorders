...
load(['output/corr_zmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_',hemi,'_all.mat'], ...
    'map','corDiag','corSigHC_P', 'corSigP_HC','repSigHC_P','repSigP_HC','siteList')
...
vari = 'corHC_P'; % 'repHC_P' , 'repP_HC', 'corHC_P', or 'corP_HC'

switch vari
    case 'corHC_P'
        corSig = corSigHC_P;
        corDiagNull = corsigmapSurrsHC_PVerAll;
        map.sigmap = map.sigmapHC_P;
    case 'corP_HC'
        corSig = corSigP_HC;
        corDiagNull = corsigmapSurrsP_HCVerAll;
        map.sigmap = map.sigmapP_HC;
    case 'repHC_P'
        corSig = repSigHC_P;
        corDiagNull = repsigmapSurrsHC_PVerAll;
        map.sigmap = map.sigmapHC_P;
    case 'repP_HC'
        corSig = repSigP_HC;
        corDiagNull = repsigmapSurrsP_HCVerAll;
        map.sigmap = map.sigmapP_HC;
    otherwise
        error('not valid');
end
...
for iDiag = 1:nDiag
    ...
    ids{iDiag}=find(tril(ones(size(corSig{iDiag})),-1));
    corToPlot{1,iDiag} = corSig{iDiag}(ids{iDiag});
    corToPlot{2,iDiag} = corDiagNull{iDiag};


    % plot table
    nSite = length(siteList{iDiag});
    hm = imagesc(ax1,1:nSite, 1:nSite, tril(corSig{iDiag},-1));
    ...
    if maxCor ~= 0
        ...
        [rowMax colMax] = find(round(maxCor,9)==round(corSig{iDiag},9));
        ...
        for iSite = 1:length(colLeft)
            ...
            patch(ax3, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', map.sigmap(iMap,:)', ...
        ...
        %   a4 = annotation(fig,'arrow',[ax1.Position(1)+ax1.Position(3)/length(corSig{iDiag})*(colLeft(1)-0.5) ax3.Position(1)-ax3.Position(3)*0.1],...
        % [ax1.Position(2)+ax1.Position(4)/length(corSig{iDiag})*(length(corSig{iDiag})-colLeft(2)+0.5) ax3.Position(2)+ax3.Position(4)*0],'LineWidth',0.8);
    ...
    [rowMin colMin] = find(round(minCor,9)==round(corSig{iDiag},9));
    ...
    for iSite = 1:length(colRight)
        ...
        patch(ax5, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', map.sigmap(iMap,:)', ...
            'EdgeColor', 'none', 'FaceColor', 'interp');
...
for iDiag = 1:nDiag
 ...
 a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.07+(iDiag-1)*0.165), ax2.Position(2)+ax2.Position(4)*0.3, 0.09, 0.02], 'string', '$ \neq $', 'edgecolor', 'none', ...
...
switch vari
    case 'corHC_P'
        savefig(fig,['output/figure_corr_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_HC_P.fig']);
        set(fig, 'PaperPositionMode', 'auto')
        print(fig, '-djpeg', '-r1200', ['output/figure_corr_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_HC_P.jpg'])
    case 'corP_HC'
        savefig(fig,['output/figure_corr_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_P_HC.fig']);
        set(fig, 'PaperPositionMode', 'auto')
        print(fig, '-djpeg', '-r1200', ['output/figure_corr_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_P_HC.jpg'])
    case 'repHC_P'
        savefig(fig,['output/figure_rep_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_HC_P.fig']);
        set(fig, 'PaperPositionMode', 'auto')
        print(fig, '-djpeg', '-r1200', ['output/figure_rep_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_HC_P.jpg'])
    case 'repP_HC'
        savefig(fig,['output/figure_rep_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_P_HC.fig']);
        set(fig, 'PaperPositionMode', 'auto')
        print(fig, '-djpeg', '-r1200', ['output/figure_rep_zmap_thres_combat',char(num2str(iCOMBAT)),'_',hemi,'_smooth',num2str(smoothKernel),'_P_HC.jpg'])
    otherwise
        error('not valid');
end

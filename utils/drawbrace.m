function h = drawbrace(ax, start, stop, width, varargin)
...
%       H = drawbrace([0 0], [1 1], 20, 'Color', 'k')
    ...
    ylims = ylim;%cellfun(@(x) str2num(x),ylim);
    xlims = xlim;%cellfun(@(x) str2num(x),xlim);
    
    % % Take logarithmic scale into account
    % isxlog = strcmp(xscale(gca), 'log');
    % isylog = strcmp(yscale(gca), 'log');
    % if isxlog
    %     start(1) = log(start(1));
    %     stop(1) = log(stop(1));
    %     xlims = log(xlims);

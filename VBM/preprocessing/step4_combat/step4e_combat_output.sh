#!/bin/bash
# Dispatcher: reads combat groups from config, submits step4e_sub_combat_output.sh per group

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SUB_SCRIPT="${SCRIPT_DIR}/step4e_sub_combat_output.sh"
...
for GROUP in $COMBAT_GROUPS; do
    echo "=== Submitting step4e for group: $GROUP ==="
    export CONFIG_FILE GROUP HPC_ENABLED

    if [[ "$HPC_ENABLED" == "1" ]]; then
        sbatch --job-name="step4e_${GROUP}" "$SUB_SCRIPT"
    else
        bash "$SUB_SCRIPT"
    fi
done

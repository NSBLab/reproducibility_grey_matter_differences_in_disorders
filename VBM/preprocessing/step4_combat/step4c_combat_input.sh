#!/bin/bash
# Dispatcher: reads combat groups from config, submits step4c_sub_combat_input.sh per group

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SUB_SCRIPT="${SCRIPT_DIR}/step4c_sub_combat_input.sh"
...
for GROUP in $COMBAT_GROUPS; do
    echo "=== Submitting step4c for group: $GROUP ==="
    export CONFIG_FILE GROUP HPC_ENABLED

    if [[ "$HPC_ENABLED" == "1" ]]; then
        sbatch --job-name="step4c_${GROUP}" "$SUB_SCRIPT"
    else
        bash "$SUB_SCRIPT"
    fi
done

    ...
    while IFS= read -r SUBJECT; do
        ...
        if [ -d "$SUBJECT_DIR" ]; then
            ...
            if [ -n "$SESSION_DIRS" ]; then
                ...
                export ses="$SES"
				FILENAME="${SUBJECT}_${SES}_T1w.nii"
				FILENAMEMW="mwp1${SUBJECT}_${SES}_T1w"
				FILENAMEP="p0${SUBJECT}_${SES}_T1w"
				FILENAMEPDIR="$DATASET_DIR/$SUBJECT/$SES/anat/p0${SUBJECT}_${SES}_T1w.nii"
				FILEDIR="$DATASET_DIR/$SUBJECT/$SES/anat/$FILENAME"
            else
				...
				FILENAMEPDIR="$DATASET_DIR/$SUBJECT/anat/p0${SUBJECT}_T1w.nii"

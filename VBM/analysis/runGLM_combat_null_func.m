function runGLM_combat_null_func(datasets,isses,smoothKernel,maskDiag, iNull,randomNumber)
% This script
...
outDir = ['/scratch2/kg98/trangc/VBM/data/derivatives/s', num2str(smoothKernel),'COMBATnull/' ,num2str(iNull),'/'];
if ~exist(outDir,'dir')
    mkdir(outDir);
end
maskDir = [inDir, 'derivatives/s', num2str(smoothKernel),'/mask_',maskDiag,'/' ];
TIVDir = [inDir, 'derivatives/s', num2str(smoothKernel),'/' ];
tiv_filename = [TIVDir, dataset, '_TIV.txt']
...
for s = 1:numSite
    ...
    for i=1:numPatGroups % For each unique patient group
        ...
        % randomise patients and controls
        ...
        newSubFolder = [outDir,diagnosisName,'/',siteName];
          ...
          delete(fullfile(newSubFolder,'*'));
            ...
            factorial_design_ttest_combat_job(newSubFolder, hcNullCell, patNullCell, age, sex, tiv, maskDir);

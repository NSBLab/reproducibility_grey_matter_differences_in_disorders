function figure_cor_tmap_raincloud_combine_thres(config)
...
load(fullfile(plot_data_dir, ['corr_tmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'.mat']), ...
...
load(fullfile(plot_data_dir, ['tmap_null_brainsmash_COMBAT',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_ver_all.mat']),...
...
paralist = {'corHC_P', 'corP_HC', 'repHC_P' , 'repP_HC' , 'corFWEHC_P', 'corFWEP_HC', 'repFWEHC_P' , 'repFWEP_HC' };
...
iData = plotorder(iDiag); % the order in the data
    ...
    corToPlot{1,iDiag} = corThres1{iData}(ids{iDiag});
    corToPlot{2,iDiag} = corsigmapSurrsHC_PVerAll{iData};

    ids{iDiag}=find(tril(ones(size(corThres2{iData})),-1));
    corToPlot{3,iDiag} = corThres2{iData}(ids{iDiag});
    corToPlot{4,iDiag} = corsigmapSurrsP_HCVerAll{iData};

    ids{iDiag}=find(tril(ones(size(repThres1{iData})),-1));
    corToPlot{5,iDiag} = repThres1{iData}(ids{iDiag});
    corToPlot{6,iDiag} = repsigmapSurrsHC_PVerAll{iData};

    ids{iDiag}=find(tril(ones(size(repThres2{iData})),-1));
    corToPlot{7,iDiag} = repThres2{iData}(ids{iDiag});
    corToPlot{8,iDiag} = repsigmapSurrsP_HCVerAll{iData};

    ids{iDiag}=find(tril(ones(size(corThresFWE1{iData})),-1));
    corToPlot{9,iDiag} = corThresFWE1{iData}(ids{iDiag});
    corToPlot{10,iDiag} = corsigFwemapSurrsHC_PVerAll{iData};

    ids{iDiag}=find(tril(ones(size(corThresFWE2{iData})),-1));
    corToPlot{11,iDiag} = corThresFWE2{iData}(ids{iDiag});
    corToPlot{12,iDiag} = corsigFwemapSurrsP_HCVerAll{iData};

    ids{iDiag}=find(tril(ones(size(repThresFWE1{iData})),-1));
    corToPlot{13,iDiag} = repThresFWE1{iData}(ids{iDiag});
    corToPlot{14,iDiag} = repsigFwemapSurrsHC_PVerAll{iData};

    ids{iDiag}=find(tril(ones(size(repThresFWE2{iData})),-1));
    corToPlot{15,iDiag} = repThresFWE2{iData}(ids{iDiag});
    corToPlot{16,iDiag} = repsigFwemapSurrsP_HCVerAll{iData};
...
for iRow = 1:numRow
    ...
    violinplot_subclass(corToPlot((1+(iRow-1)*4):(4+(iRow-1)*4),:), diagnosisStr(plotorder),'ViolinColor',ViolinColor,'BoxColor',[0 0 0],'BoxWidth',0.07);
    %[1 3 5 2 4 6]
 ...
 for iP = 1:2
        for iDiag = 1:nDiag
            if p_value((iRow-1)*2+iP,iDiag) <= thres
                a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.04+(iP-1)*0.069+(iDiag-1)*0.167), ax2.Position(2)+ax2.Position(4)*0.6, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
    ...
    a17 = annotation(fig, 'textbox', [ax3.Position(1)-0.02, ax3.Position(2), ax3.Position(3)*3, ax3.Position(4)], 'string', 'controls > cases', 'edgecolor', 'none', ...
    ...
    y = ax2.Position(2)+ax2.Position(4)*0.75;        % normalized vertical location

    % Draw three lines to make the bracket
    annotation('line', [x(1) x(1)], [y y+0.01], 'Color', 'k', 'LineWidth', 1);
    ax3= annotation('line', [x(1) x(2)], [y+0.01 y+0.01], 'Color', 'k', 'LineWidth', 1);
    annotation('line', [x(2) x(2)], [y+0.01 y], 'Color', 'k', 'LineWidth', 1);
    a17 = annotation(fig, 'textbox', [ax3.Position(1)-0.02, ax3.Position(2)+0.03, ax3.Position(3)*3, ax3.Position(4)], 'string', 'controls < cases', 'edgecolor', 'none', ...
        'FontName',font_name,'FontSize',fontsize_legend,  'horizontalalignment', 'left','Interpreter','none');

end
end
a25 = annotation(fig, 'textbox', [0.01, 0.96, 0.02, 0.02], 'string', 'a|Uncorrected', 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'left');

a26 = annotation(fig, 'textbox', [0.01, 0.72, 0.03, 0.02], 'string', 'b|Uncorrected', 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'left');

a25 = annotation(fig, 'textbox', [0.01, 0.5, 0.02, 0.02], 'string', 'c|Cluster-corrected', 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'left');

a26 = annotation(fig, 'textbox', [0.01, 0.27, 0.03, 0.02], 'string', 'd|Cluster-corrected', 'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size,  'horizontalalignment', 'left');
%%
savefig(fig,fullfile(output_dir, ['figure_corr_tmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_thres_combine.fig']));
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', fullfile(output_dir, ['figure_corr_tmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_thres_combine.jpg']))
end

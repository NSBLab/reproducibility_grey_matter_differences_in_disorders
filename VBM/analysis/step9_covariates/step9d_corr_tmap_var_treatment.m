function step9d_corr_tmap_var_treatment(config, iter, type)
% STEP9D: Medication/treatment rate between sites vs t-map correlation.
% Usage: step9d_corr_tmap_var_treatment(config [, iter, type]). Prereq: step8a.
% Writes: confound_treatment.mat under results/VBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_treatment(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_treatment.mat'), 'varTable', 'nSite');
end

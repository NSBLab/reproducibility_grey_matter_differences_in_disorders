function step9a_corr_tmap_var_age(config, iter, type)
% STEP9A: Age confound (mean/variance between sites vs t-map correlation).
%
% Usage: step9a_corr_tmap_var_age('config_hpc.json')
%        step9a_corr_tmap_var_age(config, 5000, 'spearman')
% Prereq: step8a_corr_tmap (corr_tmap_combat*_smooth*.mat on dataset_root).
% Reads:  metadataVBM_extended.csv, corr mat from results/VBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_age(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_age.mat'), 'varTable', 'nSite');
end

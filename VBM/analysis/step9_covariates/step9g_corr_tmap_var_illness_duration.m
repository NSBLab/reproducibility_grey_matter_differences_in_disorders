function step9g_corr_tmap_var_illness_duration(config, iter, type)
% STEP9G: Illness duration mean/variance between sites vs t-map correlation.
% Usage: step9g_corr_tmap_var_illness_duration(config [, iter, type]). Prereq: step8a.
% Writes: confound_illnessDuration.mat under results/VBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_illness(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_illnessDuration.mat'), 'varTable', 'nSite');
end

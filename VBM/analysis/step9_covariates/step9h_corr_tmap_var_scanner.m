function step9h_corr_tmap_var_scanner(config, iter, type)
% STEP9H: Scanner manufacturer similarity between sites vs t-map correlation.
% Usage: step9h_corr_tmap_var_scanner(config [, iter, type])
% Reads scanner_site_detailed.csv on dataset_root. Writes: confound_scanner.mat.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_scanner(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_scanner.mat'), 'varTable', 'nSite');
end

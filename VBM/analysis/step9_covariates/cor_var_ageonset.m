function [varTable, nEffect] = cor_var_ageonset(metadata, diagnosisString, corTmap1, corTmap2, siteList, iter, type)
% Computes the correlation between site-level age-onset statistics and
...
%                      and variance of age-onset
%   nEffect          : Number of sites contributing to the analysis


nSite = length(siteList); % Total number of unique sites

% Initialize age-onset mean and variance for each site
for iSite = 1:nSite
    % Compute mean of age-onset for valid entries (0 < ageOnset < 61)
    ...
    % Compute variance of age-onset for valid entries
...
if ~all(rows_with_nan)
    % Compute correlation between age-onset metrics and t-maps

function step9j_corr_tmap_var_volume(config, iter, type)
% STEP9J: Voxel size/volume between sites vs t-map correlation.
% Usage: step9j_corr_tmap_var_volume(config [, iter, type])
% Reads scanner_site_detailed.csv on dataset_root. Writes: confound_vol.mat.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_volume(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_vol.mat'), 'varTable', 'nSite');
end

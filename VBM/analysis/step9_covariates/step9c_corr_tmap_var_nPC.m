function step9c_corr_tmap_var_nPC(config, iter, type)
% STEP9C: Patient/HC/sample counts between sites vs t-map correlation.
% Usage: step9c_corr_tmap_var_nPC(config [, iter, type]). Prereq: step8a.
% Writes: confound_nPC.mat under results/VBM/analysis/output/.
...
nSite = cell(nDiag, 1);
for iDiag = 1:nDiag
    [varTable{iDiag}, nSite{iDiag}] = cor_var_nPC(metadata, diagnosisString(iDiag), cor1{iDiag}, cor2{iDiag}, siteList{iDiag}, iter, type);
...
warning('off', 'last');
save(fullfile(output_dir, 'confound_nPC.mat'), 'varTable', 'nSite');
end

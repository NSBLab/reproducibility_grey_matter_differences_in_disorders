...
load(['output/corr_tmap_parc_',num2str(nParc),'.mat'],'cor1', 't1All','siteList');
load(['output/corr_null_tmap_parc_',num2str(nParc),'.mat'], 'corNull');
corDiag = cor1;
corDiagNull = corNull;
makeUvalue = 0;
markColorbar = 1;
...
for iDiag = 1:nDiag
 ...
 a1 = annotation(fig,'rectangle',[(ax1.Position(1)+ax1.Position(3)/nSite*(colLeft(1)-1)) (ax1.Position(2)+ax1.Position(4)/nSite*(nSite-colLeft(2))) ax1.Position(3)/nSite ax1.Position(4)/nSite],'EdgeColor',darkorange,'LineWidth',1);
    ...
    if iDiag == 1
        c2 = colorbar('Position', [initX*1.6, initY, 0.01, lengthY],'AxisLocation','in','Ticks',[-2 2],'TickLabels',{'increased \newline in patients' 'reduced \newline in patients'})
        c2.Label.String = 'map';
        c2.Label.Rotation = 0;
...
%%
savefig(fig,['output/figure_corr_tmap_parc_',char(num2str(nParc)),'_combat.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', ['output/figure_corr_tmap_parc_',char(num2str(nParc)),'combat.jpg'])

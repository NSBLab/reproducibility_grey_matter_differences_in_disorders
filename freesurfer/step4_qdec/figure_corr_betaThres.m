...
ids=find(triu(ones(size(corDiagBetaThres{plotOrder(iDiag)})),1));
[fi xi]=ksdensity(corDiagBetaThres{plotOrder(iDiag)}(ids),'function','pdf','npoints',200);
...
hm = imagesc(1:length(siteString), 1:length(siteString), triu(corDiagBetaThres{plotOrder(iDiag)},1));
clim([-0.2 0.4]);
...
%%
savefig(fig,['figure_corr_mbm_thres.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r600', 'figure_corr_mbm_thres.jpg')
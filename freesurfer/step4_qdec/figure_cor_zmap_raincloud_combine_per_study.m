...
% vertices = vertices';
% faces = faces';
paralist = {'corDiag','corHC_P', 'repHC_P' ,'corDiag', 'corHC_P','repHC_P' };
nPara = length(paralist);


s_all = load(['output/corr_zmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_',hemi,'_all.mat'], 'map','corDiag', 'corSigHC_P', 'repSigHC_P' ,'siteList')

s_study = load(['output/corr_per_study_zmap_combat',num2str(iCOMBAT),'_smooth',num2str(smoothKernel),'_',hemi,'_all.mat'], 'map','corDiag','corSigHC_P', 'repSigHC_P' )
...
for iDiag = 1:nDiag
    ...
    ids = find(tril(ones(size(s_all.corDiag{iData})),-1));
    corToPlot{1,iDiag} = s_all.corDiag{iData}(ids);
    corToPlot{3,iDiag} = s_all.corSigHC_P{iData}(ids);
    corToPlot{5,iDiag} = s_all.repSigHC_P{iData}(ids);

    nStudyToPlot = sum(arrayfun(@(x)~isempty(s_study.corDiag{iData,x}), 1:size(s_study.corDiag(iData,:),2)));
    corDiagcombinePerStudy = [];
    corHC_PcombinePerStudy = [];
    repHC_PcombinePerStudy = [];
    for iStudy = 1:nStudyToPlot
        ids = find(tril(ones(size(s_study.corDiag{iData,iStudy})),-1));
        corDiagcombinePerStudy = [corDiagcombinePerStudy;s_study.corDiag{iData,iStudy}(ids)];
        corHC_PcombinePerStudy = [corHC_PcombinePerStudy;s_study.corSigHC_P{iData,iStudy}(ids)];
        repHC_PcombinePerStudy = [repHC_PcombinePerStudy;s_study.repSigHC_P{iData,iStudy}(ids)];
    end
    if isempty(corDiagcombinePerStudy)
        corToPlot{2,iDiag} = 0;
        corToPlot{4,iDiag} = 0;
        corToPlot{6,iDiag} = 0;
    else
        corToPlot{2,iDiag} = corDiagcombinePerStudy;
        corToPlot{4,iDiag} = corHC_PcombinePerStudy;
        corToPlot{6,iDiag} = repHC_PcombinePerStudy;
...
legend([dummyplot1,dummyplot2],{'all sites','within study'},'Location','northeast')
xlim([0 tempXlim(2)])
ylim(tempYlim)
...
%             a17 = annotation(fig, 'textbox', [ax2.Position(1)+ax2.Position(3)*(0.024+(iPara-1)*2*0.031+(iDiag-1)*0.166), ax2.Position(2)+ax2.Position(4)*0.31, 0.09, 0.02], 'string', '*', 'edgecolor', 'none', ...
...
maxCor = cellfun(@(x) max(x), corToPlot);
  ...
  % Convert data coordinates to normalized figure units
    ...
    y = ax2.Position(2)+ax2.Position(4)*0.45;        % normalized vertical location
  ...
  % Convert data coordinates to normalized figure units
    ...
    y = ax2.Position(2)+ax2.Position(4)*0.3;        % normalized vertical location

    % Draw three lines to make the bracket
    annotation('line', [x(1) x(1)], [y y+0.01], 'Color', 'k', 'LineWidth', 1);
    ax3= annotation('line', [x(1) x(2)], [y y], 'Color', 'k', 'LineWidth', 1);
    annotation('line', [x(2) x(2)], [y+0.01 y], 'Color', 'k', 'LineWidth', 1);
    a17 = annotation(fig, 'textbox', [ax3.Position(1)-0.02, ax3.Position(2), ax3.Position(3)*3, ax3.Position(4)], 'string', {'binary','correlation'}, 'edgecolor', 'none', ...
        'FontName',font_name,'FontSize',fontsize_legend,  'horizontalalignment', 'left','Interpreter','none');
...
%%
savefig(fig,['output/figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_per_study.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', ['output/figure_corr_zmap_combat',char(num2str(iCOMBAT)),'_smooth',num2str(smoothKernel),'_per_study.jpg'])

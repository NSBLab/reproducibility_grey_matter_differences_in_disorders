...
ids=find(triu(ones(size(corDiagGamma{plotOrder(iDiag)})),1));
[fi xi]=ksdensity(corDiagGamma{plotOrder(iDiag)}(ids),'function','pdf','npoints',200);
...
hm = imagesc(1:length(siteString), 1:length(siteString), triu(corDiagGamma{plotOrder(iDiag)},1));
clim([-0.2 1]);
...
%%
% savefig(fig,['figure_corr_sbm.fig']);
% set(fig, 'PaperPositionMode', 'auto')
% print(fig, '-djpeg', '-r600', 'figure_corr_sbm.jpg')
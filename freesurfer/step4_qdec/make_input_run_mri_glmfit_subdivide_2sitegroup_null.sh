...
outdir=/scratch/kg98/trangc/VBM/data/derivatives/freesurfer/s${smoothKernel}noCOMBAT/diag$diag}/${hemis}/resample_2sitegroup/iSubdivide_${iSubdivide}_seed2group_${randomSubdivide}/null
cd $outdir
		
covariance1=sex
covariance2=age

array=($(ls iSubdivide_${iSubdivide}_seed2group_${randomSubdivide}_*group2.txt))
...
do
	...
	do
		...
		IFS=$'\t' read -ra parts <<< "$line"
    		echo "Input ${parts[0]}${parts[4]} diagnosis${parts[1]} ${parts[2]} ${parts[3]}" >> $qdecfile #combine subid with dataset to avoid repeated id
			
		#make list input
		echo "$datadir/${parts[4]}/derivatives/freesurfer/${parts[0]}/surf/${hemis}.${measure}.fwhm${smoothKernel}.fsaverage.combat.mgh" >> $inputfile

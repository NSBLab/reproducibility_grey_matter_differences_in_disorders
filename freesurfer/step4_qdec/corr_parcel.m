...
dataFile = readtable(fullfile(dataDir,'dataset_list_SBM.txt'),'ReadVariableNames',false);
...
qdecFolder = [map.diag{iSite},'_',map.site{iSite}, '_',measureShort,'_smooth',char(num2str(smoothKernel)),'_',hemi,'_sex_age_aparc'];
 ...
 % read maps
  ...
  map.pmap(iSite,:) = double(10.^(-abs(load_mgh(fullfile(dataDir, char(map.dataSet(iSite)), 'derivatives','freesurfer', ...
...
save('output/corr_surface_aparc.mat', 'map', 'corDiag', 'corSig','siteList');

...
ids=find(triu(ones(size(corDiagGammaThres{plotOrder(iDiag)})),1));
[fi xi]=ksdensity(corDiagGammaThres{plotOrder(iDiag)}(ids),'function','pdf','npoints',200);
...
hm = imagesc(1:length(siteString), 1:length(siteString), triu(corDiagGammaThres{plotOrder(iDiag)},1));
clim([-0.2 0.4]);
...
%%
savefig(fig,['figure_corr_sbm_thres.fig']);
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r600', 'figure_corr_sbm_thres.jpg')
...
dataDir = fullfile('/scratch/kg98/trangc/VBM/data/derivatives/freesurfer',['s',char(num2str(smoothKernel)),'noCOMBAT'],['diag',num2str(diag)],hemis,'resample_1site');
addpath(genpath('/projects/kg98/trangc/VBM/code'))

sampleSizeList = [20 40 60 80 100 200 300]%300 400 500 700 1000];
...
for iSampleSize=1:length(sampleSizeList)
    ...
    if iSampleSize==1
        ...
        patch(ax3, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', matFile.map.zmap1(:,iSampleSize), ...
            'EdgeColor', 'none', 'FaceColor', 'interp');
        view([-90 0]);
    ...
    'FontName',font_name,'FontSize',font_size, 'horizontalalignment', 'left');
        ...
        patch(ax4, 'Vertices', vertices, 'Faces', faces, 'FaceVertexCData', matFile.map.zmap2, ...
            'EdgeColor', 'none', 'FaceColor', 'interp');
        view([-90 0]);
...
a2 = annotation(fig, 'textbox', [ax4.Position(1)+ax4.Position(3) ax4.Position(2)+ax4.Position(4) 0.05 0.02], 'string', ...
    'ori', ...
    'edgecolor', 'none', ...
    'FontName',font_name,'FontSize',font_size, 'horizontalalignment', 'left');
    end
...
% xlim([-0.4 1]);
% ylim([0 25]);
plotLegend = legend(ax2,[arrayfun(@num2str,sampleSizeList,'UniformOutput',0),'max'],'Location','best')
% legend('boxoff')
xlabel(ax2,'correlation')
ylabel(ax2,{'density'})
...
%%
savefig(fig,fullfile('output',['figure_corr_zmap_resample_1site_s',char(num2str(smoothKernel)),'_noCOMBAT','_diag',num2str(diag),'_',hemis,'.fig']));
set(fig, 'PaperPositionMode', 'auto')
print(fig, '-djpeg', '-r1200', fullfile('output',['figure_corr_zmap_resample_1site_s',char(num2str(smoothKernel)),'_noCOMBAT','_diag',num2str(diag),'_',hemis,'.fig']))
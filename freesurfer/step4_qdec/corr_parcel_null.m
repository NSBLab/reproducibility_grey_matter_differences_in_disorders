...
nullDir = '/scratch/kg98/trangc/VBM/data/derivatives/freesurfer/s10COMBAT/null2';
...
for iNull = [5:300]
    for iSite = 1:nSite
       ...
       % map.sigfdrmap(iSite,:) = double(mafdr(10.^(-abs(load_mgh(fullfile(nullDir, num2str(iNull),char(map.dataSet(iSite)), ...
        %    qdecFolder, [hemi,'-Diff-1-', char(map.diag(iSite)),'-Intercept-',measure], 'sig.mgh')))))<=thres);
    ...
    save([nullDir,'/', num2str(iNull),'/corr_aparc.mat'], 'map', 'corDiag', 'corSig');
end

save(['output/corr_aparc_null.mat'], 'corDiagNull','corSigNull');
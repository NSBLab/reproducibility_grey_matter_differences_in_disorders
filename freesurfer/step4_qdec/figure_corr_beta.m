...
ids=find(triu(ones(size(corDiagBeta{plotOrder(iDiag)})),1));
[fi xi]=ksdensity(corDiagBeta{plotOrder(iDiag)}(ids),'function','pdf','npoints',200);
...
hm = imagesc(1:length(siteString), 1:length(siteString), triu(corDiagBeta{plotOrder(iDiag)},1));
clim([0.3 1]);
...
%%
% savefig(fig,['figure_corr_mbm.fig']);
% set(fig, 'PaperPositionMode', 'auto')
% print(fig, '-djpeg', '-r600', 'figure_corr_mbm.jpg')
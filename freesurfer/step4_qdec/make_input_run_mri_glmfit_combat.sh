#!/bin/bash
#SBATCH --time=1-8:10:00
#SBATCH --job-name=glm_combat
#SBATCH --account=kg98
...
ls $datadir/$DATASET/qdec_table_*.dat > $datadir/$DATASET/sitelist.txt
	...
	do
		...
		do
			...
			#make list input
			echo "$datadir/$DATASET/derivatives/freesurfer/${parts[0]}/surf/${hemis}.${measure}.fwhm${smoothKernel}.fsaverage_combat.mgh" >> $inputfile

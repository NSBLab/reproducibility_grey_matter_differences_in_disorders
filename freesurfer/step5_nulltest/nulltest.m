...
% clear all
% inJob=1;
function nulltest(iCOMBAT, hemi, smoothKernel, nTrap, inJob)
...
for iSite = 1:nSite
    ...
    % [s sigmapSurrs sigmapRot] = calc_eigenstrap(s, 'map', map.sigmap(iSite,s.mask==1)','nSurrogates',nTrap,'save',false,'resample',true);
    ...
    save(['/scratch2/kg98/trangc/VBM/data/eigentrap/',char(map.site{iSite}),'/',map.numericFolders{iSite}(1:end-4), ...
        '_combat',char(num2str(iCOMBAT)),'_', hemi, '_smooth',char(num2str(smoothKernel)),'_',char(num2str(inJob)),'.mat'], ...
        'zmapSurrs','sigmapSurrs_HC_P','sigmapSurrs_P_HC','sigFdrmapSurrs_HC_P','sigFdrmapSurrs_P_HC','sigClustermapSurrs_HC_P','sigClustermapSurrs_P_HC')
end
end

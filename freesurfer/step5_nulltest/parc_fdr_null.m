...
for iSite = 1:length(datasets)
    ...
    for iDiag = 1:nDiag
        ...
        for iFile = 1:length(filesDiag)
            ...
            load(fullfile(filesDiag(iFile).folder,filesDiag(iFile).name),'zmapSurrs','sigmapSurrs');
            mapEndIn = mapStartIn + width(zmapSurrs) -1 ;
            ...
            sigmapSurrsFull(:,s.mask==1) = sigmapSurrs';
            ...
            sigmapSurrsVer(:,mapStartIn:mapEndIn) = sigmapSurrsFull';
        ...
        save(fullfile(datadir,datasets(iSite).name,['verThresMap_',namepart(1:end-6),'.mat']), "sigmapSurrsVer");

        clear sigmapSurrsSF100 sigmapSurrsSF500 sigmapSurrsSF1000 sigmapSurrsDK sigmapSurrsVer zmapSurrsSF100 zmapSurrsSF500 zmapSurrsSF1000 zmapSurrsDK zmapSurrsVer diag
    end
end